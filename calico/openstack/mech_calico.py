...
class CalicoMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def update_floatingip(self, context):
        ...
        """
        LOG.info('UPDATE_FLOATINGIP: %s', context)

        with context.session.begin(subtransactions=True):
            port = self.db.get_port(context, context.fip_update_port_id)
            self._update_port(context, port)
    ...
    def _update_port(self, context, port):
        ...
        if not port_disabled:
            ...
            # When called directly from the core plugin, we don't have the ML2
            # plugin context.
            use_context = getattr(context, '_plugin_context', context)

            with use_context.session.begin(subtransactions=True):
                port = self.db.get_port(use_context, port['id'])
                port = self.add_extra_port_information(
                    use_context, port
                )
                profiles = self.get_security_profiles(
